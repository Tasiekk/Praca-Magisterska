...
if 1 in mgr.oceny:
    ...
    wyroznienie=input('Czy uczeń ukończył szkołę z wyróźnieniem? [T/N] ')
    while wyroznienie not in ['tak','Tak','nie','Nie','t','T','n','N']:
        wyroznienie=input('Czy uczeń ukończył szkołę z wyróźnieniem? [T/N] ')
    if wyroznienie in ['tak','Tak','t','T']:
        dodatek+=7
    konkurs=input('Czy uczeń brał udział w konkursie o zasięgu ponadwojewódzkim? [T/N] ')
    while konkurs not in ['tak','Tak','nie','Nie','t','T','n','N']:
        konkurs=input('Czy uczeń brał udział w konkursie o zasięgu ponadwojewódzkim? [T/N] ')
    if konkurs in ['tak','Tak','t','T']:
        odp1=input('Czy został finalistą konkursu przedmiotowego? [T/N] ')
        while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp1=input('Czy został finalistą konkursu przedmiotowego? [T/N] ')
        if odp1 in ['tak','Tak','t','T']:
            dodatek+=10
        odp2=input('Czy został laureatem konkursu tematycznego lub interdyscyplinarnego? [T/N] ')
        while odp2 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp2=input('Czy został laureatem konkursu tematycznego lub interdyscyplinarnego? [T/N] ')
        if odp2 in ['tak','Tak','t','T']:
            dodatek+=7
        else:
            odp3=input('Czy został finalistą konkursu tematycznego lub interdyscyplinarnego? [T/N] ')
            while odp3 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                odp3=input('Czy został finalistą konkursu tematycznego lub interdyscyplinarnego? [T/N] ')
            if odp3 in ['tak','Tak','t','T']:
                dodatek+=5
                    
    konkurs=input('Czy uczeń brał udział w konkursie o zasięgu międzynarodowym lub ogólnopolskim? [T/N] ')
    while konkurs not in ['tak','Tak','nie','Nie','t','T','n','N']:
        konkurs=input('Czy uczeń brał udział w konkursie o zasięgu międzynarodowym lub ogólnopolskim? [T/N] ')
    if konkurs in ['tak','Tak','t','T']:
        odp1=input('Czy został finalistą konkursu z przedmiotu lub przedmiotów artystycznych objętych ramowym planem nauczania szkoły artystycznej? [T/N] ')
        while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp1=input('Czy został finalistą konkursu z przedmiotu lub przedmiotów artystycznych objętych ramowym planem nauczania szkoły artystycznej? [T/N] ')
        if odp1 in ['tak','Tak','t','T']:
            dodatek+=10
        odp2=input('Czy został laureatem konkursu z przedmiotu lub przedmiotów artystycznych nieobjętych ramowym planem nauczania szkoły artystycznej? [T/N] ')
        while odp2 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp2=input('Czy został laureatem konkursu z przedmiotu lub przedmiotów artystycznych nieobjętych ramowym planem nauczania szkoły artystycznej? [T/N] ')
        if odp2 in ['tak','Tak','t','T']:
            dodatek+=4
        else:
            odp3=input('Czy został finalistą konkursu z przedmiotu lub przedmiotów artystycznych nieobjętych ramowym planem nauczania szkoły artystycznej? [T/N] ')
            while odp3 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                odp3=input('Czy został finalistą konkursu z przedmiotu lub przedmiotów artystycznych nieobjętych ramowym planem nauczania szkoły artystycznej? [T/N] ')
            if odp3 in ['tak','Tak','t','T']:
                dodatek+=3
    
    konkurs=input('Czy uczeń brał udział w konkursie o zasięgu wojewódzkim? [T/N] ')
    while konkurs not in ['tak','Tak','nie','Nie','t','T','n','N']:
        konkurs=input('Czy uczeń brał udział w konkursie o zasięgu wojewódzkim? [T/N] ')
    if konkurs in ['tak','Tak','t','T']:
        odp1=input('Czy został finalistą dwóch lub więcej konkursów przedmiotowych? [T/N] ')
        while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp1=input('Czy został finalistą dwóch lub więcej konkursów przedmiotowych? [T/N] ')
        if odp1 in ['tak','Tak','t','T']:
            dodatek+=10
        else:
            odp4=input('Czy został finalistą jednego konkursu przedmiotowego? [T/N] ')
            while odp4 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                odp4=input('Czy został finalistą jednego konkursu przedmiotowego? [T/N] ')
            if odp4 in ['tak','Tak','t','T']:
                dodatek+=7
        odp2=input('Czy został laureatem dwóch lub więcej konkursów interdyscyplinarnych lub tematycznych? [T/N] ')
        while odp2 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp2=input('Czy został laureatem dwóch lub więcej konkursów interdyscyplinarnych lub tematycznych? [T/N] ')
        if odp2 in ['tak','Tak','t','T']:
            dodatek+=7
        else:
            odp5=input('Czy został laureatem jednego konkursu interdyscyplinarnego lub tematycznego? [T/N] ')
            while odp5 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                odp5=input('Czy został laureatem jednego konkursu interdyscyplinarnego lub tematycznego? [T/N] ')
            if odp5 in ['tak','Tak','t','T']:
                dodatek+=5
            else:
                odp3=input('Czy został finalistą dwóch lub więcej konkursów interdyscyplinarnych lub tematycznych? [T/N] ')
                while odp3 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                    odp3=input('Czy został finalistą dwóch lub więcej konkursów interdyscyplinarnych lub tematycznych? [T/N] ')
                if odp3 in ['tak','Tak','t','T']:
                    dodatek+=5
                else:
                    odp6=input('Czy został finalistą jednego konkursu interdyscyplinarnego lub tematycznego? [T/N] ')
    ...
    odp1=input('międzynarodowym? [T/N] ')
    while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
        odp1=input('międzynarodowym? [T/N] ')
    if odp1 in ['tak','Tak','t','T']:
        dodatek+=4
    else:
        odp1=input('krajowym? [T/N] ')
        while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
            odp1=input('krajowym? [T/N] ')
        if odp1 in ['tak','Tak','t','T']:
            dodatek+=3
        else:
            odp1=input('wojewódzkim? [T/N] ')
            while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                odp1=input('wojewódzkim? [T/N] ')
            if odp1 in ['tak','Tak','t','T']:
                dodatek+=2
            else:
                odp1=input('powiatowym? [T/N] ')
                while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
                    odp1=input('powiatowym? [T/N] ')
                if odp1 in ['tak','Tak','t','T']:
                    dodatek+=1            
    odp1=input('Czy uczeń udzielał się na rzecz srodowiska szkolnego i był aktywny w życiu społecznym szkoły? [T/N] ')
    while odp1 not in ['tak','Tak','nie','Nie','t','T','n','N']:
        odp1=input('Czy uczeń udzielał się na rzecz srodowiska szkolnego i był aktywny w życiu społecznym szkoły? [T/N] ')
    if odp1 in ['tak','Tak','t','T']:
        dodatek+=3
    ...
    if wybor==1 or wybor==3:
        ...
        print('Szkoły: ')
        print(df4)
    porownanie=input('Czy chcesz zobaczyć porównanie wyników z przewidywanymi srednimi wynikami na rok 2018? [T/N] ')
    ...
    if porownanie in ['tak','Tak','t','T']:
        ...
        porownanie1=input('Czy chcesz zobaczyć porównanie wyników z przewidywanymi wynikami dla twojego powiatu? [T/N] ')
        ...
        if porownanie1 in ['tak','Tak','t','T']:
            ...
            if wybor == 1 or wybor == 3:
                ...
                if jezyk==1:
                    print('- uczeń zdobył',mgr.procenty[4],'punktów procentowych z języka angielskiego podczas gdy srednia w powiecie wynosi', '{0:.2f}'.format(float(mgr.PowiatyAngSredni['2018-kwadratowo'][mgr.PowiatyAngSredni.index==powiat])))
                if jezyk==2:
                    print('- uczeń zdobył',mgr.procenty[4],'punktów procentowych z języka niemieckiego podczas gdy srednia w powiecie wynosi', '{0:.2f}'.format(float(mgr.PowiatyNiemSredni['2018-kwadratowo'][mgr.PowiatyNiemSredni.index==powiat])))
            else:
                ...
                if jezyk==1:
                    print('- punkty procentowe zdobyte przez ucznia z języka angielskiego mogą się wahać pomiędzy',mgr.procentyMIN[3],'a',mgr.procentyMAX[3],'podczas gdy srednia w powiecie wynosi', '{0:.2f}'.format(float(mgr.PowiatyAngSredni['2018-kwadratowo'][mgr.PowiatyAngSredni.index==powiat])))
                if jezyk==2:
                    print('- punkty procentowe zdobyte przez ucznia z języka niemieckiego mogą się wahać pomiędzy',mgr.procentyMIN[4],'a',mgr.procentyMAX[4],'podczas gdy srednia w powiecie wynosi', '{0:.2f}'.format(float(mgr.PowiatyNiemSredni['2018-kwadratowo'][mgr.PowiatyNiemSredni.index==powiat])))
